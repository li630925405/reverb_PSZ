...
for reverb_order in [0, 3]:
    for method in methods:
        # sim.set_reverb_order(reverb_order=reverb_order)
        sim = pz.Simulation(frequencies, ls_array, mic_array, pm_angle=90, reverb_order=reverb_order)
        ...
        metrics.plot("%s Contrast" % method, "0705/%s_contrast_plot_reverb_%s_pressure.png" % (method, reverb_order), "contrast")
        metrics.plot("%s Effort" % method, "0705/%s_effot_plot_reverb_%s_pressure.png" % (method, reverb_order), "effort")
        ...
        vis_frequency = 500
        vis_tfs, grid = soundfield.visualise(sim, "0705/%s-%dHz_reverb_%s_pressure.png" % (method, vis_frequency, reverb_order), frequency=vis_frequency, sf_spacing=0.05,transfer_functions=vis_tfs, grid=grid)
