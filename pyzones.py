...
class Soundfield(Rectangle):
    ...
    def auralize(self, sim, mic_num=1):
        """
        lyz
        create a auralization of the Soundfield
        """
        fr = []  # frequency response
        # aur_mics = MicrophoneArray([Microphone(position=points[0])])
        #
        tfs = sim.calculate_transfer_functions("microphones", frequency=sim.frequencies)
        # (1590, 16, 10)
        for i in range(tfs.shape[0]):
            q_matrix = np.array(sim.ls_array.get_q(frequency_index=i))
            # q_matrix = np.ones(q_matrix.shape)
            # print(q_matrix.shape) (10,)
            p = (tfs[i] @ q_matrix[:, None]).flatten()
            fr.append(p)
        ...
        # print(cwd)
        song, sr = librosa.load("../Tom.wav")
        fr = np.array(fr)
        # print("fr: ", fr.shape)
        # input()

        # print(fr.shape) (1024, 16)
        f_song = np.fft.fft(song)

        fr0 = ndimage.zoom(fr[:, 0], f_song.shape[0] / fr.shape[0])
        song_reverb = f_song * fr0
        sf.write("reverb_bright_size3.wav", np.real(np.fft.ifft(song_reverb)), 22050)

        # print(ir.shape) (5000, 16)
        # input()
        plt.cla()
        plt.figure(figsize=(10, 5))
        plt.plot(np.fft.ifft(tfs[:, 0, 0]))
        plt.show()
        # input()
    ...
    def visualise(self, sim, graph_name, frequency=500, sf_spacing=0.1, zone_spacing=0.05, zone_alpha=2, transfer_functions=None, grid=None):
        ...
        #     # 每个点都是一个vis mic, 如果没有在这声明mic_array就默认用的是self.mic_array
        #     # 只显示一个frequency, 不像simulation初始化时是显示所有frequency 所有mic
        #     # 初始化的transfer function用来计算filter, 这里只用来visualization
        #     vis_mics = MicrophoneArray([Microphone(position=points[i]) for i in range(len(points))])
        #     tfs = sim.calculate_transfer_functions("microphones", mic_array=vis_mics, frequency=frequency)
        # else:
        ...
        # 10 ls, 4 mics per circle
        # tfs:  (159, 6591, 10)
        # [k, m, n]
        # 改后: tfs: (1, 6591, 10) 更容易理解了一点..
        # (freq, ls, mic) 
        # q: (10,): complex number
        # p: (6591,)
        ...
        ## reverb
        # 计算每个ls到每个mic的image source reverb不现实啊
        # (6591, 1)
        ...
        # phase plot
        # colours = self._axes.pcolormesh(xi, yi, zi, vmin=-0.5, vmax=0.5)
...
class Simulation:
    ...
    def __init__(self, frequencies, ls_array, mic_array, size=4, reverb_order=0, c=343, rho=1.21, target_spl=76,
        ...
        self.frequency_points = len(self.frequencies)
        ...
        self.size = size
        self.reverb_order = reverb_order

        # _tf_array_ls是从speaker到mic的tf, _tf_array_mic是从mic到speaker的tf?
    ...
    def set_reverb_order(self, reverb_order):
        self.reverb_order = reverb_order
        # 怎么没改得了?
    ...
    def calculate_transfer_functions(self, orientation, mic_array=None, frequency=None):
        ...
        ## lyz: 每次要求的frequency有多少就计算多少tf不得了
        # 干嘛非要每次都新建为self.frequencies那么大
        tf_array = np.zeros((len(k), ls_num, mic_num), complex)

        # print("mic num: ", mic_num)  16
        # print("ls num: ", ls_num)  10
        ...
        for m in range(mic_num):
            for n in range(ls_num):
                ...
                #     if r[n][m] == 0:
                #         # 是0的话就是1, 那0.001处岂不是爆炸了
                #         tf_array[i][n][m] = 1
                #     else:
                #         # 直接是频域
                #         tf_array[i][n][m] = (1 / (4 * np.pi * r[n][m])) * np.exp(-1j * k[i] * r[n][m])
                #         # (0.0014708210977742876-0.04731450707471989j)
                #         # amplitude和phase分别是什么意思?

                # print(self.reverb_order)
                ir_array = self.calculate_ir(self.ls_array[n], mic_array[m], sr=k[-1]*self.c/(2*np.pi), order=self.reverb_order)
                ...
                if type(frequency) is int:
                    # print("aaaaaaa")
                    # print(frequency)
                    # print(frequency * self.frequency_points// self.frequencies[-1])
                    ir_array = ir_array[frequency * self.frequency_points // self.frequencies[-1]]
                    # print("ir: ", ir_array)
                    # print("bbbbbbb")
                # print("f: ", k[i] * self.c/(2 * np.pi))
                # print("ir: ", ir_array.shape)
                # print("tf: ", tf_array.shape)  
                # k = 2 * pi * f / c   len(k) = len(f) = 1590
                # ls: [5000, 16, 10]
                # 必须得是ls[:, 0, 0], 因为ls[:]=ls
                # ls[:][n][m] = ls[n][m], shape=(10)
                # print("i: ", k[i])
                # print(tf_array[:, n, m])
                # print(ir_array)
                # input()
                ...
                # plt.title("ir array")
                # plt.subplot(1, 2, 2)
                # plt.plot(tf_array[:, n, m])
                # plt.title("tf array")
                # plt.show()
                # input()
                # tf_array[:, n, m] = tf_array[:, n, m] * ir_array
                
                tf_array[:, n, m] = ir_array 
    ...
    def calculate_filter_weights(self, method='BC', beta=0.01):
        ...
        if method == 'BC':
            ...
            # angle of incident for pressure matching
            ...
            for i in range(len(self.k)):
                ...
                ga = g_a[i]
                gb = g_b[i]
                h = np.concatenate((ga, gb), 0)
                
                # h = np.concatenate((g_a[i], g_b[i]), 0)
                # h: transfer function
                h_h = h.conjugate().T
                # (8, 10), (10, 8) -- 因为我就bright, dark各用了4个mic
                d = np.array(d_a + d_b)
                # d: target pressure
                # bright中target就是transfer function, dark中target几乎是0
    ...
    def calculate_metrics(self, contrast=False, effort=False, planarity=False, reproduction_error=False):
        ...
        optional arguments. Contrast, effort, planarity and reproduction_error. Be advised the reproduction error is only
    ...
    def calculate_ir(self, s, m, order=1, sr=22050):
        size = self.size
        c = 343
        sr = self.frequencies[-1]
        # print("frequency: ", self.frequencies[-1])
        sources = np.array([[s.position[0], s.position[1]]])

        for i in range(order):
            for source in sources:
                img = []
                img.append([-size - source[0], source[1]])
                img.append([size - source[0], source[1]])
                img.append([source[0], -size - source[1]])
                img.append([source[0], size - source[1]])
                img = np.array(img)
        ...
        # size=4的情况下，ir只有2000那么长，所以用5000没问题
        # sr也是5000, 配合算的frequencies是0-5000
        IR = np.zeros(self.frequency_points)
        dist = []
        for source in sources:
            dist.append(math.sqrt((source[0] - m.position[0])**2 + (source[1] - m.position[1])**2))

        for d in dist:
            IR[int(d * sr // c)] = 1 / (4 * np.pi * d) + IR[int(d * sr // c)]
...
class Metrics:
    ...
    def plot(self, graph_name, file_name, metric):
        ...
        if metric == 'contrast':
            ...
            ind = 2
        elif metric == 'reproduction error':
            ind = 3
        else:
