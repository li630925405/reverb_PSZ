...
class Soundfield(Rectangle):
    ...
    def auralize(self, sim, mic_num=1, speaker_num=1):
        """
        lyz
        ...
        fr0 = ndimage.zoom(fr[:, 0], f_song.shape[0] / 1024)
        song_reverb = f_song * fr0
        sf.write("reverb.wav", np.fft.ifft(song_reverb), 22050)
        ...
        for m in range(mic_num):
            for n in range(ls_num):
                ...
                if type(frequency) is int:
                    ir_array = ir_array[frequency // 159]
