...
class Soundfield(Rectangle):
    ...
    def auralize(self, sim, mic_num=1, speaker_num=1, reverb_order=0):
        """
        lyz
        ...
        #
        for i in range(5000):
            tfs = sim.calculate_transfer_functions("microphones", frequency=i)
            q_matrix = np.array(sim.ls_array.get_q(frequency_index=i))
            p = (tfs[0] @ q_matrix[:, None]).flatten()
            tf = sim.calculate_ir(sim.ls_array, f=i, size=self._width, order=reverb_order)
            p = p * tf
            fr.append(p)
    ...
    def visualise(self, sim, graph_name, frequency=500, sf_spacing=0.1, zone_spacing=0.05, zone_alpha=2, transfer_functions=None, grid=None, reverb_order=0):
        """
        ...
        # (6591, 1)
        if reverb_order != 0:
            tf = sim.calculate_ir(sim.ls_array, vis_mics, f=1000, size=self._width, order=reverb_order)
            p = p * tf
    ...
    # def calculate_ir(self, speakers, mics, size, order=1, sr=22050, f=500):
    #     # tmp
    #     c = 343
    #     sources = None
    #     for s in speakers:
    ...
    #         else:
    #             sources = np.append(sources, [[s.position[0], s.position[1]]], axis=0)
    #     # sources = np.array([[18, 15]])
    #     # mic = [15, 15]
    #     # 共采样sr个点，这样fft后每个频点的n就是相应的频率
    #     tf = np.zeros(len(mics), dtype = 'complex_')
    ...
    #         dist = []
    #         for source in sources:
    #             dist.append(math.sqrt((source[0] - mic.position[0])**2 + (source[1] - mic.position[1])**2))
...
class Simulation:
    ...
    def calculate_filter_weights(self, method='BC', beta=0.01, reverb_order=0):
        """
        ...
        # tf_a = self.calculate_ir(self.ls_array, mics=setup_bright, f=1000, size=4, order=reverb_order)
        # tf_b = self.calculate_ir(self.ls_array, setup_dark, f=1000, size=4, order=reverb_order)
        ...
        if method == 'BC':
            for i in range(len(self.frequencies)):
                ga = g_a[i]
                ## lyz
                tfa = self.calculate_ir(self.ls_array, setup_bright, f=i, size=4, order=reverb_order)
                ga = tfa * ga
            ...
            for i in range(len(self.frequencies)):
                ga = g_a[i] * self.calculate_ir(self.ls_array, mics=setup_bright, f=i, size=4, order=reverb_order)
                gb = g_b[i] * self.calculate_ir(self.ls_array, mics=setup_dark, f=i, size=4, order=reverb_order)
    ...
    def calculate_metrics(self, contrast=False, effort=False, planarity=False, reproduction_error=False, reverb_order=0):
        """
        ...
        self.calculate_sound_pressures(tfs, eval_mics, reverb_order)
    ...
    def calculate_ir(self, speakers, size, mics=None, order=1, sr=22050, f=500):
        if mics is None:
            mics = self.mic_array
        c = 343
        sources = None
        ...
        for i, mic in enumerate(mics):
            ...
            for d in dist:
                IR[int(d * sr // c)] = 1 / d + IR[int(d * sr // c)]
            
            tf[i] = np.fft.fft(IR)[f]

        return tf

    def calculate_sound_pressures(self, tf_array, mic_array, reverb_order=0):
        """
        ...
        for i in range(len(self.frequencies)):
            ...
            if reverb_order != 0:
                # simulation里没有size这一项呃
                tf = self.calculate_ir(self.ls_array, mic_array, f=1000, size=4, order=reverb_order)
                pressures = pressures * tf
